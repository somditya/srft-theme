$(
  (function () {
    ...
    var loadNextImage = function (active) {
      var next = active.next(".img-wrapper").length
          ? active.next(".img-wrapper")
          : GALLERY.find(".img-wrapper:first"),
        nextThumb = GALLERY.find(
          '[data-thumb-id="' + next.data("img-id") + '"]'
        );

      // Setting next image & thumb properties
      GALLERY.find(".thumb").removeClass("active");
      nextThumb.addClass("active");
      //active.addClass('last-active animated rollIn');

      // Transitioning to next image & thumbnail
      scrollThumbnails(nextThumb);
      next
        .css({ opacity: 0.0 })
        .addClass("active animated rollIn")
    ...
    var loadPrevImage = function (active) {
      var prev = active.prev(".img-wrapper").length
          ? active.prev(".img-wrapper")
          : GALLERY.find(".img-wrapper:last"),
        prevThumb = GALLERY.find(
          '[data-thumb-id="' + prev.data("img-id") + '"]'
        );

      // Setting next image & thumb properties
      GALLERY.find(".thumb").removeClass("active");
      prevThumb.addClass("active animated rollIn");
      ...
      prev
        .css({ opacity: 100 })
        .addClass("active")
        .animate({ opacity: 100 }, 0, function () {
          active.removeClass("active last-active animated rollIn");
        });
    };
    ...
    var loadClickedImage = function (id) {
      ...
      image
        ...
        .addClass("active animated rollIn")
        .animate({ opacity: 1.0 }, 1000, function () {
          currActive.removeClass("last-active animated rollIn");
        });
    };
